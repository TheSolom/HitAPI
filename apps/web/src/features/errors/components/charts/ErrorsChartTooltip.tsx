interface CustomTooltipPayload {
    name?: string;
    value?: number;
    color?: string;
    dataKey?: string;
}

export interface ErrorsChartTooltipProps {
    readonly active?: boolean;
    readonly payload?: readonly CustomTooltipPayload[];
    readonly label?: string;
}

export function ErrorsChartTooltip({
    active,
    payload,
    label,
}: ErrorsChartTooltipProps) {
    if (!active || !payload || payload.length === 0) return null;

    const clientErrors =
        payload.find((p) => p.dataKey === 'clientErrors')?.value ?? 0;
    const serverErrors =
        payload.find((p) => p.dataKey === 'serverErrors')?.value ?? 0;
    const total = clientErrors + serverErrors;

    return (
        <div className="rounded-xl border border-border/80 bg-popover/95 p-3.5 shadow-xl backdrop-blur-md text-xs space-y-2 min-w-47.5">
            <div className="font-semibold text-foreground border-b pb-1.5 text-[11px] flex items-center justify-between">
                <span>{label}</span>
                <span className="text-muted-foreground font-normal">
                    {total} total errors
                </span>
            </div>
            <div className="space-y-1.5">
                <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-1.5">
                        <span className="h-2.5 w-2.5 rounded-full bg-amber-500 shadow-xs" />
                        <span className="text-muted-foreground font-medium">
                            Client (4xx):
                        </span>
                    </div>
                    <div className="flex items-center gap-1 font-semibold text-foreground">
                        <span>{clientErrors}</span>
                        {total > 0 && (
                            <span className="text-[10px] text-muted-foreground font-normal">
                                ({Math.round((clientErrors / total) * 100)}%)
                            </span>
                        )}
                    </div>
                </div>
                <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-1.5">
                        <span className="h-2.5 w-2.5 rounded-full bg-rose-500 shadow-xs" />
                        <span className="text-muted-foreground font-medium">
                            Server (5xx):
                        </span>
                    </div>
                    <div className="flex items-center gap-1 font-semibold text-foreground">
                        <span>{serverErrors}</span>
                        {total > 0 && (
                            <span className="text-[10px] text-muted-foreground font-normal">
                                ({Math.round((serverErrors / total) * 100)}%)
                            </span>
                        )}
                    </div>
                </div>
            </div>
        </div>
    );
}

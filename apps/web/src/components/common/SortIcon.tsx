...
/**
 * Generic sort direction indicator icon for table column headers.
 *
 * - Inactive column → neutral ArrowUpDown (muted).
 * - Active ASC      → ArrowUp (primary, animated).
 * - Active DESC     → ArrowDown (primary, animated).
 */
export function SortIcon({ column, sortBy, order }: Readonly<SortIconProps>) {
    if (sortBy !== column) {
        return (
